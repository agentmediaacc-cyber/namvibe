...
from services.messaging_engine import list_threads, get_thread, send_message, get_or_create_direct_thread
...
def thread(thread_id):
    profile = get_current_profile()
    thread_data = get_thread(thread_id, profile["id"])
    messages = (thread_data or {}).get("messages", [])
...
def start_chat(other_id):
    ...
    thread_id = get_or_create_direct_thread(current["id"], other_id)
    if not thread_id:
...
def api_send():
    ...
    result = send_message(thread_id, profile["id"], body, media_file)
