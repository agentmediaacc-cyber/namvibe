...
def index():
    ...
    cached = get_cache(cache_key("dashboard", profile_id))
    if cached is not None:
        return render_template('dashboard/complete_dashboard.html', **cached)

    stats = fast_query('''
        SELECT
            (SELECT COUNT(*) FROM chain_follows WHERE following_profile_id = %s AND deleted_at IS NULL) as followers,
            (SELECT COUNT(*) FROM chain_follows WHERE follower_profile_id = %s AND deleted_at IS NULL) as following,
            (SELECT COUNT(*) FROM chain_posts WHERE profile_id = %s) as posts,
            (SELECT COALESCE(SUM(likes_count), 0) FROM chain_posts WHERE profile_id = %s) as total_likes,
            (SELECT COUNT(*) FROM chain_profile_views WHERE profile_id = %s AND viewed_at > NOW() - INTERVAL '7 days') as views_last_7d
    ''', [profile_id, profile_id, profile_id, profile_id, profile_id])[0]

    engagement_rate = 0
    if stats['followers'] > 0:
        posts = fast_query('SELECT likes_count, comments_count FROM chain_posts WHERE profile_id = %s ORDER BY created_at DESC LIMIT 200', [profile_id])
    ...
    wallet = get_wallet_summary(profile_id)
    ...
    badges = []
    if viewer.get('is_verified'): badges.append({'icon': 'fa-check-circle', 'name': 'Verified', 'color': 'blue'})
    if viewer.get('is_premium'): badges.append({'icon': 'fa-crown', 'name': 'Premium', 'color': 'yellow'})
    if stats['followers'] > 1000: badges.append({'icon': 'fa-fire', 'name': 'Top Creator', 'color': 'orange'})

    settings = fast_query('''
        SELECT profile_visibility, show_activity_status, allow_messages_from, allow_video_calls
    ...
    set_cache(cache_key("dashboard", profile_id), ctx, ttl=30)
    return render_template('dashboard/complete_dashboard.html', **ctx)
