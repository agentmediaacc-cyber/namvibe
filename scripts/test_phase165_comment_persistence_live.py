#!/usr/bin/env python3
"""Phase 165 live proof for comment persistence."""
...
def login(client, profile):
    with client.session_transaction() as sess:
        ...
        sess["access_token"] = f"phase165-comment-{profile['username']}"
...
def make_photo_file():
    return (io.BytesIO(PNG_BYTES), "phase165_comment.png")
...
def main():
    print("=" * 80)
    print("PHASE 165 COMMENT PERSISTENCE LIVE")
    print("=" * 80)
    ...
    try:
        with flask_app.app_context():
            ...
            create_resp = post_multipart(
                ...
                "/api/posts/create",
                {
                    "caption": f"phase165 comment post {suffix()}",
                    "visibility": "public",
            ...
            post_row = db_one(
                "SELECT id, profile_id, comments_count, caption, media_url FROM chain_posts WHERE id = %s",
                (post_id,),
            ) if post_id else {}
            print_step(
                1,
            ...
            comment_body = f"phase165 comment {suffix()}"
            ...
            comment_db = {
                "comment_row": db_one(
                    """
                    SELECT id, profile_id, post_id, body, created_at
            ...
            print_step(
                ...
                and bool(comment_db["comment_row"].get("id")),
            )

            refresh_home = client_owner.get("/api/homepage/feed?tab=for_you&limit=20")
            refresh_home_json = refresh_home.get_json(silent=True) or {}
            refresh_detail = client_owner.get(f"/post/{post_id}")
            feed_items = ((refresh_home_json.get("payload") or {}).get("feed_items")) or []
            ...
            persistence_db = {
                ...
                "feed_post": feed_post,
                "detail_contains_count": str(comment_json.get("count")) in refresh_detail.data.decode("utf-8", "ignore"),
                "notifications": notif_rows,
            }
            print_step(
                3,
                "verify DB row + count persisted after refresh + owner notification",
                f"GET /api/homepage/feed?tab=for_you&limit=20 and GET /post/{post_id}",
                {"feed": refresh_home.status_code, "detail": refresh_detail.status_code},
                {
                    "feed_post": feed_post,
                    "detail_snippet": refresh_detail.data.decode("utf-8", "ignore")[:500],
                },
                persistence_db,
                refresh_home.status_code == 200
                and refresh_detail.status_code == 200
                and int((persistence_db["post_row"] or {}).get("comments_count") or 0) >= 1
                and int((feed_post or {}).get("comments_count") or 0) >= 1
                and any(row.get("event_type") == "comment" for row in notif_rows),
            )

            empty_resp = client_viewer.post(
                f"/api/social/post/{post_id}/comments",
                json={"body": "   "},
            )
            empty_json = empty_resp.get_json(silent=True) or {}
            empty_db = {
                "comments_count": db_one(
            ...
            print_step(
                4,
                "verify empty comment is rejected",
                f"POST /api/social/post/{post_id}/comments with empty body",
                empty_resp.status_code,
                empty_json,
                empty_db,
                empty_resp.status_code == 400 and empty_json.get("success") is False,
            )
    finally:
        cleanup(profiles)

    print("\nPASS: phase165 comment persistence live proof completed.")
    return 0
