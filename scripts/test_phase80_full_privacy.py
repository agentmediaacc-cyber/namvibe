...
FAST_MODE = os.environ.get("CHAIN_TEST_FAST", "") == "1"

if FAST_MODE:
    import services.notification_engine
    import services.socketio_service
    import services.engagement_service
    import services.follow_request_service

    services.notification_engine.create_notification = lambda *a, **kw: None
    services.follow_request_service.create_notification = lambda *a, **kw: None
    services.socketio_service.emit_to_profile = lambda *a, **kw: None
    services.socketio_service.broadcast_notification = lambda *a, **kw: None
    services.engagement_service._notify = lambda *a, **kw: False
...
def run():
    ...
    if FAST_MODE:
        print("  [FAST MODE] Notifications/sockets/cache disabled")

    print("  Step: Creating temp profiles...")
    ...
    try:
        ...
        print("  Test 10: Self check...")
        check("10. Self can always view", can_view_posts(UID_B, p_target))

    finally:
        print("  Cleanup: Removing temp profiles...")
