...
def main():
    ...
    def fake_ensure_profile(auth_user_id, defaults=None):
        ...
        row = fake_rows.get(auth_user_id) or {
            "id": str(uuid.uuid4()),
    ...
    def form_payload(index):
        suffix = uuid.uuid4().hex[:10]
        return {
            "email": f"dev_rate_{suffix}@gmail.com",
            "password": "Password123!",
            "confirm_password": "Password123!",
            "username": f"devrate{suffix[:8]}",
            "full_name": f"Dev Rate Limit {index}",
            "phone_code": "+264",
            "phone": f"81123{index:04d}",
    ...
    with patch("services.auth_service.get_supabase", return_value=supabase), \
        ...
        for index in range(1, 8):
            response = client.post("/auth/register", data=form_payload(index), follow_redirects=False)
            responses.append(response)
            print(f"post_{index}_status", response.status_code)
            print(f"post_{index}_location", response.headers.get("Location"))

    assert all(response.status_code != 429 for response in responses)
    assert responses[-1].status_code == 302, responses[-1].get_data(as_text=True)[:500]
    assert responses[-1].headers.get("Location") in {"/profile/", "/profile/onboarding"}
    ...
    print("profile_shows_not_verified", "Not Verified" in body)
