...
def forward_messages(profile_id, message_ids, to_thread_ids):
    ...
    for source_id in message_ids:
        ...
        source = None
        rows = _safe_query("SELECT * FROM chain_messages WHERE id = %s LIMIT 1", (source_id,), default=[])
        if rows:
            source = rows[0]
        else:
            for messages in _MESSAGES.values():
...
def list_shared_items(thread_id, item_type=None):
    ...
    rows = _safe_query(f"SELECT * FROM chain_message_shared_items WHERE {clause} ORDER BY created_at DESC LIMIT 100", tuple(params), default=None)
    if rows:
        return rows
