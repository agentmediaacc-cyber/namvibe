...
def _fetch_sponsored_posts():
    ...
    try:
        ...
        available = set(cols)
        where = _build_where(available)
        query = "SELECT " + ", ".join(cols) + " FROM chain_posts WHERE " + " AND ".join(where) + " ORDER BY created_at DESC NULLS LAST LIMIT 4"
        rows, _ = _run_sql("sponsored_posts", query, [])
        profile_ids = [r.get("profile_id") for r in rows if r.get("profile_id")]
        p_map = _load_profile_map(profile_ids) if profile_ids else {}
        return [{
...
def _fetch_nearby_users(current_profile=None):
    ...
    try:
        ...
        available = set(cols)
        where = _build_where(available)
        query = "SELECT " + ", ".join(cols) + " FROM chain_profiles WHERE " + " AND ".join(where) + " ORDER BY created_at DESC NULLS LAST LIMIT 4"
        rows, _ = _run_sql("nearby_users", query, [])
        return [_normalize_profile(r) for r in rows if r.get("id")]
    except Exception as e:
