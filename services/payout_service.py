...
def request_payout(creator_profile_id, amount_cents, payout_method="bank", payout_details=None):
    ...
    try:
        ...
        rate = check_action_rate_limit(creator_profile_id, "payout_request")
        if rate.get("blocked"):
    ...
    if wallet["balance_cents"] < amount_cents:
        try:
            from services.fraud_detection_service import analyze_payout_request, record_fraud_event
            from services.job_queue_service import enqueue_job
            fraud = analyze_payout_request(creator_profile_id, amount_cents, wallet.get("balance_cents", 0))
            record_fraud_event(creator_profile_id, "payout_over_balance", fraud.get("score", 0), fraud.get("severity"), metadata=fraud)
            enqueue_job("payout_review", {"creator_profile_id": creator_profile_id, "fraud": fraud}, priority=2, queue="wallet")
        except Exception:
            pass
    ...
    if available < amount_cents:
        try:
            ...
            from services.job_queue_service import enqueue_job
            fraud = analyze_payout_request(creator_profile_id, amount_cents, available)
            record_fraud_event(creator_profile_id, "payout_available_balance_risk", fraud.get("score", 0), fraud.get("severity"), metadata=fraud)
            add_to_moderation_queue(creator_profile_id, "payout", None, "fraud", fraud.get("severity", "high"), "payout_over_available_balance", {"fraud": fraud})
            enqueue_job("payout_review", {"creator_profile_id": creator_profile_id, "fraud": fraud}, priority=2, queue="wallet")
        except Exception:
            pass
    ...
    if not _db_available():
        ...
        try:
            from services.job_queue_service import enqueue_job
            enqueue_job("payout_review", {"payout_id": pid, "creator_profile_id": creator_profile_id}, priority=4, queue="wallet")
        except Exception:
            pass
