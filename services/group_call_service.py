...
def leave_group_call(call_id, profile_id):
    ...
    # Rotate encryption key when participant leaves
    if call and call.get("thread_id"):
        try:
            from services.e2ee_service import rotate_group_encryption_key
            rotate_group_encryption_key(thread_id=call["thread_id"], reason="participant_left")
        except Exception:
            pass
    return True
