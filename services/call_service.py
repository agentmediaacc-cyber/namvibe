...
def _bg_notify(receiver_profile_id, caller_profile_id, call_type, call_id, room_id):
    """Background notification creation — never blocks call start."""
    try:
...
def start_call(conversation_id, caller_profile_id, receiver_profile_id, call_type='video'):
    ...
    try:
        ...
        if receiver_profile_id:
            ...
            threading.Thread(
                target=_bg_notify,
                args=(receiver_profile_id, caller_profile_id, call_type, call_id, room_id),
                daemon=True,
            ).start()
...
def end_call(call_id, profile_id):
    ...
    if call['call_status'] == 'ringing' and str(profile_id) == str(call['caller_profile_id']):
        ...
        threading.Thread(target=_bg_notify, args=(
            call['receiver_profile_id'], call['caller_profile_id'], call['call_type'], call_id, call.get('room_id', ''),
        ), daemon=True).start()
        return None, None
...
def check_call_timeouts():
    ...
    for call in stale_calls:
        ...
        threading.Thread(
            target=_bg_notify,
            args=(call['receiver_profile_id'], call['caller_profile_id'], call['call_type'], call_id, ''),
            daemon=True,
        ).start()
