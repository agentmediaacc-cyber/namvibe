...
def get_public_groups(limit=8):
    """Fetch publicly visible groups with member counts."""
    try:
        if not _db_available():
            raise RuntimeError("db_unavailable")
        rows = fast_query(
            ...
            FROM chain_groups g
            LEFT JOIN chain_group_members gm ON g.id = gm.group_id AND gm.status = 'active'
...
def get_members(group_id, limit=50):
    group_id = _uuid(group_id)
    try:
        if not _db_available():
            raise RuntimeError("db_unavailable")
        rows = fast_query(
            "SELECT * FROM chain_group_members WHERE group_id = %s AND status = 'active' ORDER BY role ASC, joined_at DESC LIMIT %s",
            (group_id, limit), timeout_ms=800, default=[],
        )
        return rows or []
    except Exception:
        return []

def get_join_requests(group_id, status="pending"):
    group_id = _uuid(group_id)
    try:
        if not _db_available():
            raise RuntimeError("db_unavailable")
        rows = fast_query(
            "SELECT * FROM chain_group_join_requests WHERE group_id = %s AND status = %s ORDER BY created_at DESC",
            (group_id, status), timeout_ms=800, default=[],
        )
        return rows or []
    except Exception:
        return []

def approve_join_request(group_id, profile_id):
    group_id = _uuid(group_id)
    profile_id = _uuid(profile_id)
    try:
        _write("UPDATE chain_group_join_requests SET status = 'approved', updated_at = now() WHERE group_id = %s AND profile_id = %s", (group_id, profile_id))
        _write("INSERT INTO chain_group_members (group_id, profile_id, role, status) VALUES (%s, %s, 'member', 'active') ON CONFLICT DO UPDATE SET status = 'active', role = 'member'", (group_id, profile_id))
        return {"ok": True, "status": "approved", "group_id": group_id, "profile_id": profile_id}
    except Exception:
        return {"ok": False, "status": "error", "detail": "db_unavailable"}

def reject_join_request(group_id, profile_id):
    group_id = _uuid(group_id)
    profile_id = _uuid(profile_id)
    try:
        _write("UPDATE chain_group_join_requests SET status = 'rejected', updated_at = now() WHERE group_id = %s AND profile_id = %s", (group_id, profile_id))
        return {"ok": True, "status": "rejected", "group_id": group_id, "profile_id": profile_id}
    except Exception:
        return {"ok": False, "status": "error", "detail": "db_unavailable"}

def get_announcements(group_id, limit=10):
    group_id = _uuid(group_id)
    try:
        if not _db_available():
            raise RuntimeError("db_unavailable")
        rows = fast_query(
            "SELECT * FROM chain_group_announcements WHERE group_id = %s ORDER BY created_at DESC LIMIT %s",
            (group_id, limit), timeout_ms=800, default=[],
        )
        return rows or []
    except Exception:
...
def get_adverts(group_id, limit=10):
    group_id = _uuid(group_id)
    try:
        if not _db_available():
            raise RuntimeError("db_unavailable")
        rows = fast_query(
            "SELECT * FROM chain_group_adverts WHERE group_id = %s ORDER BY created_at DESC LIMIT %s",
            (group_id, limit), timeout_ms=800, default=[],
        )
        return rows or []
    except Exception:
...
def my_groups(profile_id, limit=20):
    profile_id = _uuid(profile_id)
    try:
        if not _db_available():
            raise RuntimeError("db_unavailable")
        rows = fast_query(
            ...
            FROM chain_groups g
            JOIN chain_group_members gm ON g.id = gm.group_id AND gm.profile_id = %s AND gm.status = 'active'
            ORDER BY gm.joined_at DESC
            LIMIT %s
            """,
            (profile_id, limit), timeout_ms=800, default=[],
        )
        return rows or []
    except Exception:
        return []
