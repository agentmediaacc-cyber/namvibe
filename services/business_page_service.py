...
def create_campaign(profile_id, title, objective="reach", content_url=None, media_type="image",
                    target_audience=None, budget=0, start_date=None, end_date=None, target_url=None):
    cid = str(uuid.uuid4())
    try:
        write_query(
            """INSERT INTO chain_ad_campaigns 
               (id, owner_id, title, objective, content_url, media_type, target_audience, budget, starts_at, ends_at, target_url, status)
               VALUES (%s, %s, %s, %s, %s, %s, %s::jsonb, %s, %s, %s, %s, 'pending')""",
             (cid, profile_id, title, objective, content_url, media_type,
             json.dumps(target_audience or {}), budget, start_date or datetime.now(timezone.utc), end_date, target_url)
        )
...
def get_campaigns(profile_id, limit=20):
    rows = fast_query(
        "SELECT * FROM chain_ad_campaigns WHERE owner_id = %s ORDER BY created_at DESC LIMIT %s",
...
def pause_campaign(campaign_id, profile_id):
    write_query("UPDATE chain_ad_campaigns SET status = 'paused' WHERE id = %s AND owner_id = %s",
               (campaign_id, profile_id))
    return {"ok": True}

def track_campaign_metric(campaign_id, metric="impressions", count=1):
    col_map = {"impressions": "impressions_count", "clicks": "clicks_count", "reach": "reach", "engagement": "engagement"}
    col = col_map.get(metric, "impressions_count")
